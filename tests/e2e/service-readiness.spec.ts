...
test('persists partial item receiving, replacement quantities and credits in supplier performance', async ({ page, browser }, testInfo) => {
  ...
  await receiving.getByLabel('Cumulative received', { exact: true }).fill('60');
  await receiving.getByLabel('Cumulative rejected', { exact: true }).fill('10');
  ...
  await expect(page.getByRole('heading', { name: 'Supplier performance', exact: true })).toBeVisible();
  ...
  await expect(receiving.getByLabel('Cumulative received', { exact: true })).toHaveValue('60');
  await receiving.getByLabel('Cumulative received', { exact: true }).fill('110');
...
test('creates an approved menu, saves readiness, and drafts only the stock shortage', async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', { name: 'Save and calculate readiness' }).click();
  ...
  await expect(shortage.getByRole('cell').nth(0)).toHaveText('20');
  await expect(shortage.getByRole('cell').nth(1)).toHaveText('5');
  await expect(shortage.getByRole('cell').nth(3)).toHaveText('15');
  ...
  await page.getByRole('button', { name: 'Create procurement draft' }).click();
