...
A later CI attempt passed 56 of 57 database tests but exposed an intermittent legacy query-count assertion: it counted transaction-control events along with data reads, so delayed transaction events could contaminate the next sample. The comparison now counts only SELECTs against business tables, with exact Settings expectations of three separate-strategy reads versus two joined reads and unchanged result equality. The focused reporting/Settings suite passed all three tests after correction, and three additional reporting-only runs passed with the exact counts; no production query or authorization behavior changed for this test repair.
